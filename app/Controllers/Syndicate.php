		...
		switch ($start_message) 
			{
				case 0:
					...
					// get all syndicates in syndicate name sequence - depends on project
					...
					switch ($session->current_project[0]['project_name']) 
						{
							case 'FreeBMD':
								...
								if ( $session->current_identity[0]['role_index'] <= 1 )
									{
										$parameter = $parameter_model->where('Parameter_key', 'pretendtobecoord')->find();
								...
								foreach ( $session->project_syndicates as $key => $project_syndicate )
									{
										...
										if ( $freecomett_syndicate )
											{
												$temp_synd[$key] = [
													'BMD_syndicate_credit' => $freecomett_syndicate[0]['BMD_syndicate_credit'],
													'status' => $freecomett_syndicate[0]['status'],
													'new_user_environment' => $freecomett_syndicate[0]['new_user_environment'],
													'verify_mode' => $freecomett_syndicate[0]['verify_mode'],
													];
											}
										else
											{
		...
		switch ($session->BMD_cycle_code) 
			{
				...
				case 'SYDEL': // delete user data
					...
					return redirect()->to( base_url('database/delete_user_data_step2/'.$session->BMD_identity_index));
					break;
		...
		foreach ( $transcribers_in_syndicate as $transcriber )
			{
				...
				$freecomett_identity = $identity_model
					->where('project_index',  $session->current_project[0]['project_index'])
					->where('BMD_User', $transcriber['UserID'])
					->find();
				// found?
				if ( $freecomett_identity )
					{
						$freecomett_transcribers[$transcriber['UserID']] = $freecomett_identity[0];
					}
			}
