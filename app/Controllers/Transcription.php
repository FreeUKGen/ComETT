...
class Transcription extends BaseController
{
	...
	public function reopen_BMD_step2()
	{
		...
		if ( $session->BMD_file_exists_on_project == '1')
			{	
				...
				// get all data entry definitions for this transcription.
	...
	public function freebmd_loadCSV($CSV_lines, $reopen_transcription)
	{
		...
		if ( $header_comment[1] != '' )
			{
				// chop text to 100 chars max
				$comment_chopped = substr($header_comment[1], 0, 100);
				// insert
	...
	public function freebmd_createDB($CSV_lines)
	{
		...
		$curl_url = $session->current_project[0]['project_autoimageservertype'].$session->current_project[0]['project_autoimageurl'].$session->current_allocation[0]['BMD_reference'].$scan_name;
		$ch = curl_init($session->curl_url);
		...
		if ( curl_getinfo($ch, CURLINFO_HTTP_CODE) != 200 )
			{
				curl_close($ch);
				$session->set('message_2', 'The scan '.$scan_name.' does not exist on the '.$session->current_project[0]['project_name']. 'server => '.$session->current_project[0]['project_autoimageurl'].'/'.$session->current_allocation[0]['BMD_reference'] );
				$session->set('message_class_2', 'alert alert-danger');
				return redirect()->to( base_url('transcription/download_transcription_step1/1') );
			} 	
		...
		switch ($start_message) 
			{
				case 0:
					// message defaults
					$session->set('message_1', 'Add / Change / Remove comments for this transcription => '.$session->current_transcription[0]['BMD_file_name']);
					$session->set('message_class_1', 'alert alert-primary');
					$session->set('message_2', '');
					$session->set('message_class_2', '');
