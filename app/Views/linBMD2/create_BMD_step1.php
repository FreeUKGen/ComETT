	...
	<form action="<?php echo(base_url('transcription/create_bmd_step2')) ?>" method="post">
		...
		<div class="form-group row d-flex align-items-center">
				<label for="fetch_scan" class="col-2">Automatically download the scan from FreeBMD? =></label>
				<select name="fetch_scan" id="fetch_scan" class="box col-1">
					<?php foreach ($session->yesno as $key => $value): ?>
						 <option value="<?php echo esc($key)?>"<?php if ( $key == $session->fetch_scan ) {echo esc(' selected');} ?>><?php echo esc($value)?></option>
					<?php endforeach; ?>
				</select>
				<small id="userHelp" class="form-text text-muted col-1">Select YES if you are unsure and the scan will be automatically downloaded from <?php echo $session->current_project[0]['project_name']; ?>.</small>
				<small id="userHelp" class="form-text text-muted col-2">Select NO if you want to download the scan manually. You must place it here, "/srv/freecomett/public/Users/FreeBMD/your_user_name/Scans" Normally you should never do this. Always use the YES option.</small>
		</div>
		...
		<div class="row mt-4 d-flex justify-content-between">	
				...
				<a class="btn btn-primary mr-0" href="<?=(base_url('allocation/manage_allocations/0')) ?>">Manage your <?php echo $session->current_project[0]['project_name'] ?> Allocations</a>
